...
cap = cv2.VideoCapture('areyouok.mp4')
height = cap.get(cv2.CAP_PROP_FRAME_HEIGHT)
width = cap.get(cv2.CAP_PROP_FRAME_WIDTH)
#video_writer = cv.VideoWriter("pose_estimation_demo.mp4", cv.VideoWriter_fourcc('D', 'I', 'V', 'X'), 25, (640, 480), True)
video_writer = cv2.VideoWriter("areyouok2.avi", cv2.VideoWriter_fourcc('M', 'J', 'P', 'G'), 25, (1280,720), True)
while cv2.waitKey(1) < 0:
    hasFrame, frame = cap.read()
    if not hasFrame:
        cv2.waitKey()
        break
    im=cv2.resize(frame,(1280,720))
#    cv2.resize(img,(2*width,2*height),interpolation=cv2.INTER_CUBIC)
    ...
    for i in range(len(face_loc)): # how mush face
        ...
        if point_2d[:,1].max()< 700 and point_2d[:,0].max()<1260:
